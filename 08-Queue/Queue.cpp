...
BOOL ConsumeElement(int nThreadNum, int nRequestNum, HWND hWndLB) {
	...
	// Fall asleep untill there is something to read.
...
DWORD WINAPI WriterThread(PVOID pvParam) {
	...
	for (int nRequestNum = 1; !g_fShutdown; nRequestNum++) {
		...
		if (g_q.IsFull() && !g_fShutdown) {
			...
			AddText(hWndLB, TEXT("[%d] Queue is full: impossible to add %d"), nThreadNum, nRequestNum);
		...
		if (g_fShutdown) {
			...
			// No need to keep the lock any logner
			ReleaseSRWLockExclusive(&g_srwLock);
...
BOOL Dlg_OnInitDialog(HWND hWnd, HWND hWndFocus, LPARAM lParam) {
	...
	DWORD dwThreadID;
	for (int x = 0; x < 4; x++) {
		g_hThreads[g_nNumThreads++] = chBEGINTHREADEX(NULL, 0, WriterThread, (PVOID)(INT_PTR)x, 0, &dwThreadID);
	}
...
int APIENTRY wWinMain(HINSTANCE hinstExe, HINSTANCE, PTSTR pszCmdLine, int) {

	DialogBoxW(hinstExe, MAKEINTRESOURCE(IDD_QUEUE), NULL, Dlg_Proc);
	StopProcessing();
	return(0);
}
